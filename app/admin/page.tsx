...
export default function AdminPanel() {
  ...
  const cargarUsuarios = useCallback(async () => {
    ...
    // Derivar plan desde evento_limit si columna plan no existe aún en BD
    const usuariosNorm = (data ?? []).map((u: UsuarioAdmin) => ({
      ...u,
      plan: u.plan ?? (u.evento_limit === null ? "pro" : "free"),
    }));
    setUsuarios(usuariosNorm);
    const limites: Record<string, string> = {};
    usuariosNorm.forEach((u: UsuarioAdmin) => {
  ...
  async function cambiarPlan(usuario: UsuarioAdmin) {
    ...
    // Actualizar plan + evento_limit juntos (funciona aunque columna plan no exista)
    const updatePayload: Record<string, unknown> = {
      evento_limit: nuevoPlan === "pro" ? null : PLAN_LIMIT_FREE,
    };
    // Intentar incluir columna plan (si no existe en BD, Supabase la ignorará)
    updatePayload.plan = nuevoPlan;
    const { data, error } = await supabase
      .from("profiles")
      .update(updatePayload)
      .eq("id", usuario.id)
      .select("id, plan, evento_limit");
    ...
    if (error) {
      ...
      setUsuarios((prev) =>
        prev.map((u) => u.id === usuario.id
          ? { ...u, plan: nuevoPlan, evento_limit: nuevoPlan === "pro" ? null : PLAN_LIMIT_FREE }
          : u)
      );
      setEditLimite((prev) => ({
        ...prev,
        [usuario.id]: nuevoPlan === "pro" ? "" : String(PLAN_LIMIT_FREE),
      }));
    }
        ...
        <nav className="nav">
          <div className="nav-left">
            <Link href="/dashboard" className="nav-logo">
              <AppLogo size={30} />
              <span className="nav-logo-text">Evorix</span>
            </Link>
              ...
              usuariosFiltrados.map((u) => (
                <div key={u.id} className="user-row">
                  ...
                  <div className="user-row-top">
                    ...
                    <div className="user-info">
                      <div className="user-nombre">
                        ...
                        {!u.es_admin && (
                          <span className={`badge-plan ${(u.plan === "pro" || u.evento_limit === null) ? "badge-plan-pro" : "badge-plan-free"}`}>
                            {(u.plan === "pro" || u.evento_limit === null) ? "✦ Pro" : "Free"}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="user-row-bottom">
                    ...
                    {!u.es_admin && (
                      <button
                        className={`btn-plan ${(u.plan === "pro" || u.evento_limit === null) ? "btn-plan-downgrade" : "btn-plan-upgrade"}`}
                        disabled={!!cambiandoPlan[u.id]}
                        title={(u.plan === "pro" || u.evento_limit === null) ? "Mover a Free" : `Activar Pro — sin límite de invitados`}
                        onClick={() => cambiarPlan(u)}
                      >
                        {cambiandoPlan[u.id] ? "..." : (u.plan === "pro" || u.evento_limit === null) ? "↓ Mover a Free" : "✦ Activar Pro"}
                      </button>
                    )}
