...
export default function Dashboard() {
  ...
  // Recargar al volver al dashboard (para que los checkmarks se actualicen)
  useEffect(() => {
    const onFocus = () => { cargarEventos(); };
    window.addEventListener("focus", onFocus);
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "visible") cargarEventos();
    });
  ...
  async function cargarStats(eventoId: string) {
    const [invData, fotosData, deseosData, mesasData, presentesData] = await Promise.all([
      supabase
        .from("invitados")
    ...
    if (invData.data) {
      ...
      setStats((prev) => ({
        ...
        [eventoId]: {
          ...
          total_presentes: presentesData.count ?? 0,
        },
      }));
    }
  }
  ...
  useEffect(() => {
    ...
    document.title = "Evorix — Dashboard";
    setMounted(true);
    checkUser();
    cargarEventos();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:0;border-top:1px solid var(--rule);background:var(--paper)}
        .pa-link{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:7px;padding:14px 4px;text-decoration:none;color:var(--ink-soft);font-family:var(--sans);font-size:10px;font-weight:600;letter-spacing:1.3px;text-transform:uppercase;border-right:1px solid var(--rule);border-bottom:1px solid var(--rule);transition:all .2s var(--ease);background:transparent;position:relative;-webkit-tap-highlight-color:transparent;min-height:72px}
        .pa-link:nth-child(3n){border-right:none}
        .pa-link:nth-child(7),.pa-link:nth-child(8),.pa-link:nth-child(9){border-bottom:none}
                    ...
                    <article key={evento.id} className="event-card">
                      ...
                      <div className="nav-grid">
                        <Link href={`/eventos/${evento.id}/editar`} className="pa-link pa-link-edit">
                          <Icon.pencil />
                          {t.edit}
                          {(evento.imagen_url || evento.frase_evento || evento.mensaje_invitacion || evento.musica_url || (evento.plantilla && evento.plantilla !== "clasica") || (evento.color_primario && evento.color_primario !== "#0D9488")) && <span className="pa-check">✓</span>}
                        </Link>
                        ...
                        <Link href={`/muro/${evento.id}`} className="pa-link">
                          <Icon.wall />
                          {t.wall}
                          {s && s.total_fotos > 0 && <span className="pa-check">✓</span>}
                        </Link>
                        ...
                        <Link href={`/eventos/${evento.id}/configurar`} className="pa-link">
                          <Icon.gear />
                          {t.settings}
                          {(evento.regalo_activo || evento.vestimenta_activo) && <span className="pa-check">✓</span>}
                        </Link>
                        <Link href={`/libro/${evento.id}`} className="pa-link">
                          <Icon.book />
                          {t.book}
                          {s && s.total_deseos > 0 && <span className="pa-check">✓</span>}
                        </Link>
                        <Link href={`/eventos/${evento.id}/agradecimientos`} className="pa-link">
                          <Icon.mail />
                          {t.thanks}
                          {evento.agradecimiento_enviado && <span className="pa-check">✓</span>}
                        </Link>
                        <Link href={`/eventos/${evento.id}/scanner`} className="pa-link">
                          <Icon.scanner />
                          {t.scanner}
