...
export default function ScannerPage() {
  ...
  // jsQR se carga dinámicamente — siempre habilitado como fallback universal
  const jsQRRef = useRef<((data: Uint8ClampedArray, w: number, h: number) => {data:string}|null)|null>(null);
  useEffect(() => {
    setBarcodeSupport(true); // siempre true: usamos BarcodeDetector si existe, jsQR si no
  ...
  // Loop de escaneo: BarcodeDetector (Chrome) o jsQR (todos los demás)
  const iniciarScanLoop = useCallback(() => {
    ...
    async function loop() {
      ...
      try {
        ...
        if (detector) {
          // Chrome: BarcodeDetector nativo
          const barcodes = await detector.detect(video);
          if (barcodes.length > 0) raw = barcodes[0].rawValue;
        } else if (jsQRRef.current && canvas) {
          // Fallback universal: jsQR
  ...
  useEffect(() => {
    if (scanning) iniciarScanLoop();
    return () => cancelAnimationFrame(scanLoopRef.current);
  }, [scanning, iniciarScanLoop]);
  ...
  async function procesarToken(raw: string) {
    ...
    if (!token) {
      setResultado({ status: "error", message: "QR no reconocido. Asegúrate de escanear el QR de Evorix." });
      return;
    }
    setProcesando(true);
    try {
  ...
  return (
    <div className="page">
      ...
      <div className="wrap">
        ...
        {/* Aviso de carga de jsQR (solo se muestra si aún no cargó) */}
        {camPermiso === "ok" && false && (
